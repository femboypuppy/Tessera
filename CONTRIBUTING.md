...
`pnpm dev` starts the web app with hot reload and prints its address.

### Everyday commands

| Command                      | Does                                                          |
| ---------------------------- | ------------------------------------------------------------- |
| `pnpm dev`                   | The web app with hot reload.                                  |
...
### The server and the desktop app

```bash
DATA_DIR=./data pnpm --filter @tessera/server dev   # the sync server
pnpm --filter @tessera/desktop tauri dev            # the desktop app (needs Rust, see below)
```
...
- **Packages don't import each other**, except `core` and `ui`. That keeps features independent.
