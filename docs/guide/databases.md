...
| Formula       | A value computed from other properties. See [Formulas](#formulas).     |
...
Add a **Formula** property and write an expression that reads other properties (click one in the
list to insert it) and combines them with functions: logic (`if`, `and`, `or`, `not`, `empty`),
math (`round`, `abs`, `min`, `max`, `sum`, …), text (`concat`, `contains`, `upper`, `slice`, …) and
dates (`now`, `dateAdd`, `dateBetween`, `formatDate`, …). The editor shows a live preview and
points at the exact position of any error.
