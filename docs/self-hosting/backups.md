...
`tessera-server backup` writes one `.tar.gz` file with a consistent copy of the database and every
attachment, while the server keeps running:

```bash
docker exec tessera tessera-server backup /data/backups/tessera-$(date +%F).tar.gz
```
...
15 3 * * * docker exec tessera tessera-server backup /data/backups/tessera-$(date +\%F).tar.gz && docker exec tessera sh -c 'ls -1t /data/backups/*.tar.gz | tail -n +15 | xargs -r rm --'
```
...
1. Copy the backup file into `./backups` next to `docker-compose.yml`, then stop the server:
   `docker compose down`.
2. Restore, then start the server:

```bash
docker compose run --rm -v "$PWD/backups:/backups" tessera tessera-server restore /backups/tessera-2026-09-23.tar.gz
docker compose up -d
```

Restore checks the archive and the database's integrity, then moves whatever is currently in
`DATA_DIR` to `DATA_DIR/pre-restore-<timestamp>/` before replacing it, so nothing is ever deleted
outright: you don't need to start from an empty volume.
