 ...
 *
 * What it expects from the server (apps/server):
 * - `GET /api/health` answers 200 `{ ok: true }` (the Docker HEALTHCHECK uses it too);
 * - `GET /` serves the web app;
 * - the owner is created with `tessera-server create-owner --email E --name N --password-stdin`
 *   (falling back to `POST /api/auth/setup { setupCode, email, password, name }`, the first-run form,
 *   with the setup code the server logged);
 ...
 *   bearer token; documents are named `<workspaceId>/ws:<workspaceId>` (SMOKE_DOC_NAME, `{id}`
 *   placeholder). Every assumption is in the `server` object below.
 *
 * `--allow-stub` is for a server that only answers /api/health (the pre-merge stub): the script
 * then checks what the image itself is responsible for (health, non-root user, writable /data
 * surviving a restart, graceful stop) and reports the owner and sync steps as skipped.
 */
...
const server = {
  ...
  docName: (workspaceId) =>
    (process.env.SMOKE_DOC_NAME ?? '{id}/ws:{id}').replaceAll('{id}', workspaceId),
  /** Run with `docker exec -i`; the password goes to stdin. */
  createOwnerCli: [
    'tessera-server',
    'create-owner',
    '--email',
  ...
  async createOwnerHttp() {
    ...
    const setupCode = /"setupCode":"([^"]+)"/.exec(`${logs.stdout}${logs.stderr}`)?.[1];
    return fetch(`${base}/api/auth/setup`, json({ ...owner, setupCode: setupCode ?? '' }));
  },
  async signIn() {
...
function dockerWithInput(input, ...commandArgs) {
  return spawnSync('docker', commandArgs, { encoding: 'utf8', input });
}
...
try {
  ...
  if (!isStub) {
    ...
    await step('create the owner', async () => {
      const cli = dockerWithInput(
        `${owner.password}
`,
        'exec',
        '-i',
        container,
        ...server.createOwnerCli,
      );
      if (cli.status === 0) return;
      const response = await server.createOwnerHttp();
      if (!response.ok)
        throw new Error(
          `create-owner failed (${cli.stderr.trim()}) and POST /api/auth/setup answered ${response.status}`,
        );
    });
