...
describe('KeychainCredentialStore', () => {
  ...
  it('is the app credential store inside the desktop app (priority 100)', async () => {
    const { createDesktopFeature } = await import('../index');
    const service = createDesktopFeature().services?.find(
      (entry) => entry.provides === 'credentialStore',
    );
    expect(service).toMatchObject({ id: 'keychain', priority: 100 });
    const store = (await service?.create({} as never)) as KeychainCredentialStore;
    expect(store).toBeInstanceOf(KeychainCredentialStore);
    await store.set('https://notes.example.com', 'tsk_2');
    expect(fakeTauri().state.secrets).toEqual({ 'https://notes.example.com': 'tsk_2' });
  });
