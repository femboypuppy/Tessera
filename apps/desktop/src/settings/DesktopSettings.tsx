...
function ServersSection() {
  const ctx = useAppContext();
  // The keychain store (priority 100): the tokens the sync feature signed in with.
  const store = ctx.services.credentialStore;
  const [servers, setServers] = useState<ServerEntry[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const load = useCallback(() => {
    store.list().then(setServers, (err: unknown) => setError(toError(err).message));
  }, [store]);
  useEffect(load, [load]);
