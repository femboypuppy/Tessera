...
export async function activateSync(ctx: AppContext): Promise<() => void> {
  ...
  // Sign-in (Settings → Sync & account) and the provider keep bearer tokens in the resolved
  // credential store: the OS keychain on the desktop.
  if (ctx.platform.isDesktopApp)
    (await import('../client/connection')).setCredentialStore(ctx.services.credentialStore);
