...
export async function exportMarkdown(
  ...
  // 4. Page contents, read once up front: a block ID is only written where a link in the export
  // points at it (the editor gives most blocks an ID; ` ^id` on every line would clutter the files).
  const contents = new Map<string, { doc: DocJSON; props: PageProps }>();
  ...
  for (const item of items) {
    ...
    try {
      ...
      try {
        ...
        contents.set(item.page.id, { doc, props: getPageProps(handle.doc) });
        for (const link of extractLinks(doc)) if (link.blockRef) linkedBlocks.add(link.blockRef);
      } finally {
        handle.release();
      }
    } catch (error) {
  ...
  const keepBlockId = (blockId: string) => linkedBlocks.has(blockId);
  ...
  async function readPage(pageId: string) {
    const cached = contents.get(pageId);
    if (cached) {
      contents.delete(pageId);
      return cached;
    }
  ...
  async function writePage(item: Item) {
    const { doc, props } = await readPage(item.page.id);
