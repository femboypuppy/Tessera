...
describe('markdown export', () => {
  ...
  it('writes Obsidian markdown: frontmatter, shortest-path wikilinks, embeds, callouts', () => {
    ...
    // A block ID is written where a link points at it (^step-2, from Welcome), not on blocks
    // nothing links to (^step-1): the editor gives most blocks an ID, which would clutter the files.
    expect(plan).toContain('2. Board the crew ^step-2');
    expect(plan).toContain('1. Fuel the rocket\n');
    expect(plan).not.toContain('^step-1');
