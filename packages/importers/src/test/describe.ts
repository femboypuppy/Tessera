 ...
 * compare equal. Block IDs are kept where a link in the tree points at them, the ones a markdown
 * export writes; the rest are internal (the editor gives most blocks one).
 */
export async function describeTree(
  ...
  for (const page of snapshot.descendants(rootId)) {
    ...
    for (const link of extractLinks(await docOf(ctx, page.id)))
      if (link.blockRef) linkedBlocks.add(link.blockRef);
  }
  const replaceIds = (node: AnyNodeJSON): AnyNodeJSON => {
    const attrs = node.attrs ? { ...node.attrs } : undefined;
    if (attrs && typeof attrs.blockId === 'string' && !linkedBlocks.has(attrs.blockId))
      attrs.blockId = null;
