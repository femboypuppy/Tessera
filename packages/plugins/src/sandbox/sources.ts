...
const NONCE = /^[A-Za-z0-9+/_-]{8,}={0,2}$/;
/** In `index.html` until the server or the desktop app replaces it (no app policy then). */
const NONCE_PLACEHOLDER = '__TAURI_SCRIPT_NONCE__';

/**
 * The nonce the app's own Content-Security-Policy trusts, if it has one. Sandbox documents are
 * `srcdoc` frames, which inherit the app's policy on top of their own, so under a strict app
 * policy their bootstrap only runs with its nonce. The Tessera server and the desktop app (Tauri)
 * write the page's nonce into `<meta property="csp-nonce">`; a nonce'd script works too.
 */
export function appCspNonce(doc: Document | undefined = globalThis.document): string | null {
  if (!doc) return null;
  const meta = doc.querySelector<HTMLMetaElement>('meta[property="csp-nonce"]');
  // Browsers hide the attribute once a policy applies; the `nonce` property keeps the value.
  const candidates = [meta?.nonce || meta?.getAttribute('nonce')];
  for (const script of Array.from(doc.scripts)) candidates.push(script.nonce);
  for (const value of candidates)
    if (value && value !== NONCE_PLACEHOLDER && NONCE.test(value)) return value;
  return null;
}

/** The CSP nonce of a sandbox document: the app's (see {@link appCspNonce}), else a random one. */
export function createNonce(): string {
  const app = appCspNonce();
  if (app) return app;
