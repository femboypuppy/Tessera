...
/** A search hit with the extras the palette and the search page use. */
export interface RichSearchHit extends SearchHit {
  /** Slug-free heading text when the best match is a heading (navigate to it). */
  heading?: string;
  /** Block to scroll to for body matches, when the block has an ID. */
  blockId?: string;
