...
export class RemarkMarkdownCodec implements MarkdownCodec {
  ...
  serialize(doc: DocJSON, options: MarkdownSerializeOptions = {}): string {
    const normalized = normalizeDocJSON(doc);
    const kept = options.keepBlockId
      ? withoutBlockIds(normalized, options.keepBlockId)
      : normalized;
    const tree = docToMdast(kept, options);
...
/** A copy of `doc` whose block IDs are cleared unless `keep` accepts them. */
function withoutBlockIds(doc: DocJSON, keep: (blockId: string) => boolean): DocJSON {
  const visit = (node: AnyNodeJSON): AnyNodeJSON => {
    const blockId = node.attrs?.blockId;
    const attrs =
      typeof blockId === 'string' && !keep(blockId) ? { ...node.attrs, blockId: null } : node.attrs;
  ...
  return visit(doc as unknown as AnyNodeJSON) as unknown as DocJSON;
}
