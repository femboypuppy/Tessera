...
test('desktop screenshots', async ({ page, context }) => {
  ...
  for (const line of [
    'The program that landed twelve people on the Moon between 1969 and 1972.',
    '## Milestones',
    '- Apollo 8: the first crew to orbit the Moon, Christmas 1968',
    'Apollo 11: the first landing, in the Sea of Tranquility',
    'Apollo 13: the successful failure, and a crew brought home safely',
    '',
    '## This week',
    'Review the lunar module checklist with the crew before Thursday’s simulation.',
  ]) {
    await page.keyboard.type(line);
    await page.keyboard.press('Enter');
  }
  await page.keyboard.press('Backspace');
  await expect(body.getByRole('heading', { name: 'Milestones' })).toBeVisible();
