...
export const test = base.extend<{ debugHooks: void }, { syncServer: SyncServer }>({
  syncServer: [
    ...
    // Starting the server (tsx, SQLite, migrations) gets its own budget, as in the testkit's
    // fixture: otherwise its seconds count against the first test of each worker.
